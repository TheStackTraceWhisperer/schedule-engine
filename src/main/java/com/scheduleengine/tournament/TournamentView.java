...
public class TournamentView {
    ...
    public VBox getView() {
        ...
        clearFilter.setOnAction(e -> { typeFilter.setValue(null); loadData(); });
    ...
    private GridPane buildTournamentForm(Tournament existing) {
        ...
        leagueCombo.setPromptText("(Optional - for League Scoped)");
    ...
    private void showRegisterDialog(Tournament tournament) {
        ...
        teamCombo.setCellFactory(lv -> new ListCell<>() {
        ...
        dialog.setResultConverter(btn -> {
            if (btn == registerButtonType) {
                if (teamCombo.getValue() == null) {
                    showError("Validation", "Please select a team");
                    return null;
                }
                try {
                    return registrationService.registerTeam(tournament, teamCombo.getValue(), notesField.getText());
        ...
        dialog.showAndWait().ifPresent(reg -> {
            ...
            a.setHeaderText("Team Registered");
            String msg = "Team '" + reg.getTeam().getName() + "' registered for tournament.\nStatus: " + reg.getStatus();
