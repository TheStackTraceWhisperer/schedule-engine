...
public class TournamentDetailView {
    ...
    private void showEditDialog(Tournament existing) {
        ...
        grid.add(new Label("Entry Fee:"), 0, row); grid.add(entryFeeField, 1, row++);
        ...
        dialog.setResultConverter(btn -> {
            if (btn == saveButtonType) {
                ...
                try { existing.setEntryFee(Double.parseDouble(entryFeeField.getText())); } catch (NumberFormatException e) { existing.setEntryFee(0.0); }
