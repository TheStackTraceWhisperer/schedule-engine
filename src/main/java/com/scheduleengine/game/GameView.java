...
public class GameView {
  ...
  public VBox getView() {
    ...
    scoreCol.setComparator((a, b) -> {
      int ai = TableColumnUtil.parseLeadingInt(a);
      int bi = TableColumnUtil.parseLeadingInt(b);
    ...
    table.getColumns().addAll(idCol, dateCol, statusCol, homeCol, awayCol, fieldCol, seasonCol, actionCol);

    // Persist table state (widths, visibility, sort)
    com.scheduleengine.common.TablePreferencesUtil.bind(table, "game");
    com.scheduleengine.common.TablePreferencesUtil.attachToggleMenu(table, "game");
  ...
  private void loadData() {
    ...
    if (selectedLeague != null) {
      list = list.stream().filter(g -> {
        ...
        return g.getAwayTeam() != null && g.getAwayTeam().getLeague() != null && selectedLeague.getId().equals(g.getAwayTeam().getLeague().getId());
      }).toList();
    }
  ...
  public GridPane buildGameForm(Game existing) {
    ...
    homeCombo.setConverter(teamConv);
    awayCombo.setConverter(teamConv);
    homeCombo.setCellFactory(lv -> new ListCell<>() {
      @Override
    ...
    awayCombo.setCellFactory(lv -> new ListCell<>() {
      @Override
      protected void updateItem(Team item, boolean empty) {
        super.updateItem(item, empty);
        setText(empty || item == null ? null : item.getName());
    ...
    seasonCombo.setConverter(new StringConverter<>() {
      @Override
      public String toString(Season s) {
        return s == null ? "" : s.getName();
      }
    ...
    seasonCombo.setCellFactory(lv -> new ListCell<>() {
      @Override
      protected void updateItem(Season item, boolean empty) {
        super.updateItem(item, empty);
        setText(empty || item == null ? null : item.getName());
