...
public class PaymentsView {
    ...
    private ObservableList<Transaction.Status> selectedStatuses;
    ...
    private void loadFilters() {
        ...
        if (!statuses.isBlank()) {
            for (String s : statuses.split(",")) {
                try { selectedStatuses.add(Transaction.Status.valueOf(s)); } catch (IllegalArgumentException ignored) {}
            }
        }
    ...
    public VBox getView() {
        ...
        TableColumn<Transaction, String> playerCol = new TableColumn<>("Player");
        playerCol.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getPlayerName() != null ? c.getValue().getPlayerName() : ""));
        ...
        amountCol.setOnEditCommit(e -> {
            try {
                e.getRowValue().setAmount(Double.parseDouble(e.getNewValue()));
                transactionService.save(e.getRowValue());
            } catch (Exception ex) {
        ...
        HBox controls = new HBox(10, addBtn, resetFiltersBtn, paidBtn, voidBtn, importBtn, exportBtn);
    ...
    private void createColumnHeaderFilters(
            TableColumn<Transaction, String> partyTypeCol,
            TableColumn<Transaction, String> partyNameCol,
            TableColumn<Transaction, String> leagueCol,
            TableColumn<Transaction, String> teamCol,
            TableColumn<Transaction, String> playerCol,
            TableColumn<Transaction, String> categoryCol,
            TableColumn<Transaction, String> dateCol,
            TableColumn<Transaction, String> amountCol,
            TableColumn<Transaction, String> statusCol,
        ...
        filterPartyType.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        ...
        filterPartyName.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        ...
        filterLeagueName.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        leagueCol.setText("");
        ...
        filterTeamName.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        teamCol.setText("");
        teamCol.setGraphic(teamNameHeader);

        // Player Name Filter
        filterPlayerName = new TextField();
        filterPlayerName.setPromptText("Player");
        VBox playerNameHeader = createHeaderWithFilter(playerCol, filterPlayerName);
        filterPlayerName.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        playerCol.setText("");
        ...
        filterCategory.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        categoryCol.setText("");
        ...
        filterDate.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        dateCol.setText("");
        ...
        filterAmount.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        amountCol.setText("");
        ...
        filterNotes.textProperty().addListener((obs, o, n) -> { applyFilters(); saveFilters(); });
        notesCol.setText("");
        notesCol.setGraphic(notesHeader);
    }

    private VBox createHeaderWithFilter(TableColumn<Transaction, String> column, Control filterControl) {
    ...
    private void showCreateLeagueInvoiceDialog() {
        ...
        SearchableComboBox<Team> teamCombo = new SearchableComboBox<>();
        teamCombo.setPromptText("Select Team");

        leagueCombo.setOnAction(e -> {
            League selectedLeague = leagueCombo.getValue();
            if (selectedLeague != null) {
                teamCombo.setItems(FXCollections.observableArrayList(teamService.findByLeague(selectedLeague)));
            }
        });
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == saveButtonType) {
                try {
                    ...
                    tx.setNotes("League registration: " + leagueCombo.getValue().getName() + " - " + notesField.getText());
                    return tx;
                } catch (Exception e) {
                    showError("Invalid input: " + e.getMessage());
                    return null;
    ...
    private void showCreateTournamentInvoiceDialog() {
        ...
        SearchableComboBox<Team> teamCombo = new SearchableComboBox<>(FXCollections.observableArrayList(teamService.findAll()));
        teamCombo.setPromptText("Select Team");
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == saveButtonType) {
                try {
                    ...
                    tx.setAmount(Double.parseDouble(amountField.getText()));
                    tx.setStatus(Transaction.Status.PENDING);
                    tx.setNotes("Tournament registration: " + tournamentCombo.getValue().getName() + " - " + notesField.getText());
                    return tx;
                } catch (Exception e) {
                    showError("Invalid input: " + e.getMessage());
                    return null;
    ...
    private static class PartyOption {
        final Long id;
        final String name;

        PartyOption(Long id, String name) {
            this.id = id;
            this.name = name;
        }
